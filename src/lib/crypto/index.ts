...
export async function deriveKey(password: string, saltHex: string): Promise<CryptoKey> {
	...
	const passKey = await crypto.subtle.importKey('raw', encoder.encode(password), 'PBKDF2', false, [
		'deriveKey'
	]);
...
export async function decryptDiary(key: CryptoKey, ivHex: string, ctHex: string): Promise<string> {
